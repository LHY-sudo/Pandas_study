...
```python
In : left = pd.DataFrame({'key': ['foo', 'bar'], 'lval': [1, 2]})

In : right = pd.DataFrame({'key': ['foo', 'bar'], 'rval': [4, 5]})
...
In : df = pd.DataFrame(np.random.randn(8, 2), index=index, columns=['A', 'B'])
In : df2 = df[:4]
In : df2
Out: 
                     A         B
...
In : ts = pd.Series(np.random.randint(0, 500, len(rng)), index=rng)
...
```python
In : df = pd.DataFrame({"id": [1, 2, 3, 4, 5, 6],
...
```python
In : ts = pd.Series(np.random.randn(1000),
...
In : ts.plot()
Out: <matplotlib.axes._subplots.AxesSubplot at 0x7f2b5771ac88>
```

`DataFrame` 的 `plot()` 方法可以快速绘制所有带标签的列：

```python
In : df = pd.DataFrame(np.random.randn(1000, 4), index=ts.index,
...:                   columns=['A', 'B', 'C', 'D'])
...
In : df.plot()
Out: <matplotlib.axes._subplots.AxesSubplot at 0x7f2b53a2d7f0>
In : plt.legend(loc='best')
Out: <matplotlib.legend.Legend at 0x7f2b539728d0>
```
